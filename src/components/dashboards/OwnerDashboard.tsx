...
import { 
  ...
  Calendar, ArrowRight, Download, Filter, PlayCircle, AlertCircle, Briefcase
} from 'lucide-react';
import { 
...
export default function OwnerDashboard({ onChannelSelect }: { onChannelSelect?: (channel: any) => void }) {
  const { user, switchUser } = useAuth();
  const { tasks, channels, users, completedTasks: completedTasksData, stageEvents, otEntries } = useData();
    ...
    <div className="min-h-screen p-6 md:p-8">
      ...
      <div className="max-w-[1800px] mx-auto space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3 md:gap-4">
          ...
          <Card className="stat-card-liquid card-blue fade-in p-4 md:p-5 cursor-pointer hover:scale-105 transition-transform" style={{ animationDelay: '0.25s' }}>
            <div className="flex items-center gap-2 mb-2">
          ...
          <Card className="stat-card-liquid card-orange fade-in p-4 md:p-5 cursor-pointer hover:scale-105 transition-transform" style={{ animationDelay: '0.3s' }}>
            <div className="flex items-center gap-2 mb-2">
        ...
        <div className="glass-card p-6 md:p-8 fade-in">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-2xl font-bold">OT Insights</h2>
            </div>
            <Button 
              onClick={() => {
                ...
                const a = document.createElement('a');
                a.href = url;
                a.download = `ot-report-${format(new Date(), 'yyyy-MM')}.csv`;
                a.click();

                toast({
                  title: "Exported",
                  description: "OT report exported to CSV.",
                });
              }}
              className="btn-secondary-liquid"
            >
          ...
          {(() => {
            const otThisMonth = otEntries.filter(e => 
              e.date >= monthStart && e.date <= monthEnd
            );
            const otLastMonth = otEntries.filter(e => {
              const lastMonthStart = startOfMonth(subDays(monthStart, 1));
              const lastMonthEnd = endOfMonth(subDays(monthStart, 1));
              return e.date >= lastMonthStart && e.date <= lastMonthEnd;
            });
            const totalOTThisMonth = otThisMonth.reduce((sum, e) => sum + e.amount, 0);
            const totalOTLastMonth = otLastMonth.reduce((sum, e) => sum + e.amount, 0);
            const otTrend = totalOTThisMonth - totalOTLastMonth;

            // Top OT claimers
            const otByEmployee = users
              .filter(u => u.role !== 'owner')
              .map(user => {
                const userOT = otThisMonth.filter(e => e.userId === user.id);
            ...
            // OT trend (last 30 days)
            const last30Days = Array.from({ length: 30 }, (_, i) => {
              const date = subDays(now, 29 - i);
              const dayOT = otEntries.filter(e => 
            ...
            // Channel-wise OT
            const otByChannel = channels.map(channel => {
              const channelOT = otThisMonth.filter(e => e.channelId === channel.id);
              const total = channelOT.reduce((sum, e) => sum + e.amount, 0);
              return { channel: channel.name, total, count: channelOT.length };
            }).filter(c => c.count > 0).sort((a, b) => b.total - a.total);

            // Average OT per employee
            const avgOTPerEmployee = otByEmployee.length > 0
              ? (totalOTThisMonth / otByEmployee.length).toFixed(0)
              : 0;

            // Pending approvals (if applicable)
            const pendingOT = 0; // Would need approval status field
              ...
              <>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                  <Card className="stat-card-liquid card-orange p-4">
                    ...
                    <p className="text-2xl md:text-3xl font-bold text-white mb-1">₹{totalOTThisMonth.toLocaleString()}</p>
                    <TrendIndicator value={otTrend} />
                  </Card>
                  ...
                  <Card className="stat-card-liquid card-blue p-4">
                    ...
                    <p className="text-2xl md:text-3xl font-bold text-white">{otByEmployee.length}</p>
                  ...
                  <Card className="stat-card-liquid card-purple p-4">
                    <div className="flex items-center gap-2 mb-2">
                      <BarChart3 className="h-4 w-4 text-purple-300" />
                      <p className="text-xs text-white/70 font-semibold uppercase">Avg/Employee</p>
                    </div>
                    <p className="text-2xl md:text-3xl font-bold text-white">₹{avgOTPerEmployee}</p>
                  </Card>
                  ...
                  <Card className="stat-card-liquid card-cyan p-4">
                    ...
                    <p className="text-2xl md:text-3xl font-bold text-white">
                      {otThisMonth.reduce((sum, e) => sum + (e.type === 'full_day' ? 8 : 4), 0)}h
                    </p>
                  </Card>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {/* Top OT Claimers */}
                  <div>
                    <h3 className="text-lg font-bold mb-4">Top OT Claimers</h3>
                    <div className="space-y-3">
                      {otByEmployee.map((emp, idx) => (
                  ...
                  {/* Channel-wise OT */}
                  <div>
                    <h3 className="text-lg font-bold mb-4">Channel-wise Breakdown</h3>
                    <div className="space-y-4">
                      {otByChannel.map((ch, idx) => (
                        <div key={idx}>
                          <div className="flex justify-between text-sm mb-2">
                            <span className="font-medium">{ch.channel}</span>
                            <span className="text-muted">₹{ch.total.toLocaleString()} • {ch.count} entries</span>
                          </div>
                          <div className="progress-bar-liquid">
                            <div 
                              className="progress-fill-liquid bg-gradient-to-r from-orange-500 to-orange-400" 
                              style={{ width: `${(ch.total / totalOTThisMonth) * 100}%` }}
                            />
                          </div>
                        </div>
                      ))}
                ...
                {/* OT Trend Graph */}
                <div className="mt-6">
                  <h3 className="text-lg font-bold mb-4">OT Trend (Last 30 Days)</h3>
